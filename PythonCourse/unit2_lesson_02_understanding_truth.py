...
-as we check in C that if we consider an empty array or empty string,they return false,but they are not equal to false,they only refered to be false same as in python empty string,list,tuple etc
-but 0 is directly equal to false and 1 equals true,we can check them in python console by int(True) and int(False)
-i had understood how and which are referred to true or false eg,,,,all NoneType are referred false and None is the first class object in python
"""

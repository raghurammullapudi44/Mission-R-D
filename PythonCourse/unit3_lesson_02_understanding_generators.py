...
def test_evens():
    gen = evens()
    for num in xrange(1000):   # we can also use xrange(0,1000,2) here without using if conditional
        if num%2==0:
            assert num == gen.next()
    for num in xrange(1000):
        if num%2==0:
            assert num+1000 == gen.next()
...
-what is a generator and how it use ful in creating data a required point of time
-how range() and xrange() differs
-how can we generate large data with generators with out overflowError because generator does not allocate memory,comparitve to range() function
"""

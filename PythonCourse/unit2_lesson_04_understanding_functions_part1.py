...
def get_word_with_least_vowels(input):
    ...
    """
    #pass(input)# replace this with right code. Use min builtin and define a new function to pass as key.
    if type(input).__name__ != 'list':
        input=list(input)
    vowels=vowels_count(input)
    leastvowel=dict(zip(vowels,input))
    return leastvowel[min(leastvowel.keys())]

def vowels_count(input):
    vowels=[]
    for i in xrange(len(input)):
        vowels.append(len(list(set(['a','e','i','o','u'])&set(input[i]))))
    return vowels
...
def get_min_max_words(input):
    ...
    if len(input) == 1:
        ...
        least_highest=dict(zip(letters,input))
        return least_highest[min(least_highest.keys())],least_highest[max(least_highest.keys())]

def letter_count(input):
    letters=[]
    letters.append(len(input[0]))
    letters.append(len(input[1]))
    letters.append(len(input[2]))
    return letters

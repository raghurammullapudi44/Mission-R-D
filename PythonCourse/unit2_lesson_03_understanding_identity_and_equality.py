...
-differences between identity and equality,in sence identity means it refers to same object we can check it by id(__) in pytho console when the identity is true they both refer to same location,so any operation on one effets the other
-identity shows different behaviour for larger numbers and smaller numbers when they are referred to different objects,they are identical if numbers are small and they are not identical if the numbers are large
-for None both equality and identity remains same
"""

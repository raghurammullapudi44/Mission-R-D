...
def test_generator_zip_student():
    pass
    gen = generator_zip(range(5), range(4), range(5), range(6))
    assert type(gen).__name__ == 'generator'
    check_generator(gen, 4, 4)

    gen = generator_zip(range(5), range(4), range(3))
    assert type(gen).__name__ == 'generator'
    check_generator(gen, 3, 3)

    gen = generator_zip(range(5), range(4), range(5), range(6), range(7), range(3))
    assert type(gen).__name__ == 'generator'
    check_generator(gen, 3, 6)

    gen = generator_zip(range(5), range(2), range(1))
    assert type(gen).__name__ == 'generator'
    check_generator(gen, 1, 3)

    gen = generator_zip(range(1, 5), "abc", [1])
    assert [(1, 'a', 1)] == list(gen)

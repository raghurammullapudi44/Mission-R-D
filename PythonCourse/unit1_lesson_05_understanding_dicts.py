...
def test_dictionary_is_mutable():
    ...
    word_to_digit["three"] = 3          #we can append to the dictionary by using this
    assert {'three': 3, 'two': 2, 'one': 1} == word_to_digit

    del word_to_digit["one"]            #we can delete from the dictionary by using this
...
def test_valid_dictionary_keys():
    ...
    try:
        key = [1]                      #but if key = 1 or anything there will be no exception
        test_dict[key] = "list"
    except TypeError as te:
        print te  #observe the error message. #unhasable type: 'list'
        assert True

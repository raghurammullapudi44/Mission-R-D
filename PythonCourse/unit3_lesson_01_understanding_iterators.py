...
def test_iter_on_iterator():
    fruits = ["apple", "orange", "banana"]
    iter1 = iter(fruits)
    iter2 = iter(iter1)   #this is equivalent to iter1=iter2=iter(fruits) and implementation on any one effects the other as the share same id() location
...
def test_for():
    ...
    for x in iter1:
        result.append(x)
    assert [] == result  #what happened here? here the list is empty because next() method is called for each iteration in for loop,so it next method returns StopIteration because in the above for loop all the elements are returned
...
def test_find_builtins_that_work_on_iterables():
    funcs = ['abs()','divmod()','input()','open()','staticmethod()','all()','enumerate()','int()','ord()','str()','any()','eval()','isinstance()','pow()','sum()','basestring()','execfile()','issubclass()','print()','super()','bin()','file()','iter()','property()','tuple()','bool()','filter()','len()','range()','type()','bytearray()','float()','list()','raw_input()','unichr()','callable()','format()','locals()','reduce()','unicode()','chr()','frozenset()','long()','reload()','vars()','classmethod()','getattr()','map()','repr()','xrange()','cmp()','globals()','max()','reversed()','zip()','compile()','hasattr()','memoryview()','round()','__import__()','complex()','hash()','min()','set()','apply()','delattr()','help()','next()','setattr()','buffer()','dict()','hex()','object()','slice()','coerce()','dir()','id()','oct()','sorted()','intern()']
    assert 80 == len(funcs)
...
def my_map(func, iterable):
    #pass
    result=[]
    if 'list' == type(iterable).__name__:
        result=appending(func,iterable)

    elif 'str' == type(iterable).__name__:
        x=97
        while x in range(97,123):
            result.append(func(chr(x)))
            x += 1

    elif 'listiterator' == type(iterable).__name__:
        result=appending(func,iterable)

    return result

def appending(func,iterable):
    x=0
    result=[]
    while x in iterable:
...
-what is an iterable
-how it can be used with various iterables and next() method and when iterable returns StopIteration
-and how to implement map function with out using its built-in
"""
...
def test_raw_strings_are_strings():
    assert True == isinstance(r"Hello World", str)  # if r is used it is called raw string and if we use backslash two bacckslahes will be printed and and next literal escapes,if we use u or U it will become uncode string,if we use b or B it indicates the literal should be in bytes
...
def test_triple_quoted_strings_can_span_lines():
    tq_str = """Hello
World"""
    dq_str = "Hello"   # what is the double quoted form of tq_str
    assert False == (tq_str == dq_str)
...
def test_string_membership():
    assert 0 == ('c' in 'apple')  #is there a precedence issue here?
    assert 1 == ('a' in 'apple')
    assert 1 == ('app' in 'apple')
